...
- `network-calls.test.cjs` walks the TypeScript AST of `src/**` and fails on any network API other than `fetch`. Each `fetch` must be one of:
  - `` fetch(`${BFF_URL}/<path>`, { method: '<LIT>' }) `` hitting a contract operation, in a file whose `BFF_URL` comes from `BFF_USER_API_URL`;
  ...
  The expected server and browser call lists are pinned, so adding a network call means updating this test.
...
`docker compose watch` (needs `NODE_AUTH_TOKEN` exported, passed as a build secret) starts Postgres + Liquibase (`dev-latest` GHCR images), Redis, Core API (host 3000), BFF User (host 4000) and this front built from `development.Dockerfile` in `next dev` mode, published on host **3001** (container port 3000), with `./src` synced. Set `PROJECT_FRONT_URL` for the post-login redirect.
...
- **BFF URL** — `BFF_USER_API_URL` → `USER_BFF_URL` (fallback `http://localhost:4000`). The resolution is duplicated in `bff-proxy.ts` (`configuredBffUrl`, read per request) and at module scope in both auth route handlers: change all three together. The `@mairie360/bff-user-openapi` dependency is not imported anywhere; types come from `src/contracts/bff.d.ts`.
...
- `src/app/api/auth/login/route.ts`: validates the credentials with the same rules as BFF User's `LoginView` (`EMAIL_PATTERN` is the `z.email()` regex, non-empty password) so it never sends an out-of-contract request, then sends `email`, `password` and the user-agent as `device_info` to BFF User `/auth/login` (10 s timeout, 502 vs 504 distinguished). Error responses carry front-owned French messages chosen by status: BFF User's `message` values are technical English (`Invalid login payload`, `Upstream service error`) and are never shown. On success the **access token is taken only from the upstream `Authorization: Bearer` header** (never from a refresh token in the body) and stored in the `accessToken` cookie: HttpOnly, SameSite strict, path `/`, 24 h, Secure in production, `COOKIE_DOMAIN` when set (must match the other fronts so they can read it).
